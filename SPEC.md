...
- **Interruptor:** «Comprobar el sitio antes de registrar en Git», activado de fábrica y persistido (`docrecorder.verifyBeforeCommit`). Hay tandas en las que se prefiere registrar y revisar después, y forzar la espera sería peor.
...
## 21. Rama por guía y pie de Git plegable

Petición del usuario: al terminar una guía y empezar otra, la nueva se montaba en la rama de la anterior; y el bloque de Git y comprobaciones ocupaba media columna y estorbaba para desplazarse por las capturas.

- **Nombre automático por guía o por módulo** (`gitBranchMode`, persistido en `docrecorder.branchMode`, de fábrica `guide`). `suggestBranchName(módulo, funcionalidad, modo)`: por guía da `docs/<módulo>-<funcionalidad>` —guion y no barra, porque Git no admite `docs/x` y `docs/x/y` a la vez y hay repositorios con ramas por módulo—; sin funcionalidad escrita se enseña `docs/<módulo>-…` (sin funcionalidad no se puede guardar). Por módulo, `docs/<módulo>` como antes. La rama elegida (`gitBranchOverride`) manda sobre ambos; «volver al nombre automático» la olvida.
- **`targetBranch(s)`** en el store es el único cálculo de la rama de destino: lo usan la franja, el selector, el pie, el aviso de ■, el guardado y la ficha de requisitos (que antes enseñaba la rama de HEAD, justo la de la guía anterior).
- **Al estrenar guía se pregunta** (`NextBranchDialog`) cuando la siguiente caería en la misma rama que la que se acaba de registrar (rama elegida a mano, o modo por módulo), y también al descartar una edición (que fija la rama de su commit): *Rama nueva para la guía* (`branchPerGuide`: olvida elegida y base, pasa a modo guía), *Seguir en …* o *Elegir otra rama…*. Va en la cola de avisos tras guardar (`afterSave`: rama → contexto de IA → ficha), de uno en uno.
- **`startFreshSession` vacía funcionalidad y título.** Conservarlos hacía que la guía siguiente heredara la rama por guía de la anterior y, guardada sin cambiarlos, reescribiera su carpeta.
- **Pie plegable** (`GitPanel`): envuelve `GitSection` y `DocsChecksSection`; plegado (`docrecorder.gitPanelCollapsed`) queda una línea con la rama de destino, cuántos comandos se comprobarán y el aviso de archivos ya indexados. Desplegado, su cuerpo se limita a `45vh` con desplazamiento propio.
