...
interface DocStep {
  id: string
  order: number
  action: 'click' | 'fill' | 'select' | 'submit' | 'press' | 'navigate' | 'capture' | 'image' | 'content' | 'section'
  kind?: 'interaction' | 'capture' | 'image' | 'content' | 'section' // ausente = interaction (lo grabó el motor)
